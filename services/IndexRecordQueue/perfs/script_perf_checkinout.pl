...
my $baseurl     = $ARGV[0] || "http://limoges-dev.biblibre.com/cgi-bin/koha";
#my $baseurl     = $ARGV[0] || "http://admin.koha_master.localhost/cgi-bin/koha";
my $max_tries   = 200;
my $concurrency = 1;
my $user        = $ARGV[1] || 'testtest';
my $password    = $ARGV[2] || 'testtest';
...
for my $barcode ( @barcodes4issues ) {
    ...
    my @postdata = ( "foo=foo&borrowernumber=" . $$borrower{borrowernumber} . "&barcode=$barcode&issueconfirmed=1&bar=bar" );
...
#my $solr_query = "recordtype:biblio AND int_onloan:[* TO *] AND str_barcode:";
#$solr_query .= "(" . join( ' OR ', @barcodes4issues ) . ")";
#warn 
#my $res = SimpleSearch( $solr_query, {});
#
#my @biblionumbers_indexed;
#for my $r ( @{ $res->items } ) {
#    push @biblionumbers_indexed, $$r{values}{biblionumber};
#}
#$logger and $logger->write("Total of " . scalar(@biblionumbers_indexed) . " are indexed");
#$logger and $logger->write(join(' ; ', @biblionumbers_indexed));
...
sub report_regress {
    ...
    for my $runnumber ( grep { /run/ } keys %{$regress->{regression}} ) {
        ...
        $s .= "\n";
        $total_response_time += $$run{total_response_time};
    }
    my $avg_response_time = $total_response_time / scalar( grep { /run/ } keys %{$regress->{regression}} );
    $s .= "total response time = $total_response_time\n";
    $s .= "avg response time = $avg_response_time\n\n";
    return $s;
}
