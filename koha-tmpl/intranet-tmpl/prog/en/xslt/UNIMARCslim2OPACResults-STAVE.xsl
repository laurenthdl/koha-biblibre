...
<xsl:import href="UNIMARCslimUtils-STAVE.xsl"/>
...
<tr>
  ...
  <xsl:variable name="ccode" select="marc:datafield[@tag=099]/marc:subfield[@code='t']"/>
  <td width="35">
  <xsl:for-each select="marc:datafield[@tag=099]">
	 <xsl:if test="$ccode='DVD'">
		<img src="/opac-tmpl/prog/itemtypeimg/bridge/dvd.gif" />
	</xsl:if>
	 <xsl:if test="$ccode='Livre'">
		<img src="/opac-tmpl/prog/itemtypeimg/bridge/book.gif" />
	 </xsl:if>
	  <xsl:if test="$ccode='CD'">
		<img src="/opac-tmpl/prog/itemtypeimg/bridge/cd_music.gif" />
	 </xsl:if>
	  <xsl:if test="$ccode='Revue'">
		<img src="/opac-tmpl/prog/itemtypeimg/bridge/periodical.gif" />
	 </xsl:if>
	  <xsl:if test="$ccode='Multimedia'">
		<img src="/opac-tmpl/prog/itemtypeimg/bridge/laserdisk.gif" />
	 </xsl:if>
	 <xsl:if test="$ccode='Article'">
		<img src="/opac-tmpl/prog/itemtypeimg/npl/SIRS.gif" />
	 </xsl:if>
	 <xsl:text>     </xsl:text>
      </xsl:for-each>
     </td> 
  ...
  <span class="results_summary">
        ...
        <span class="available">
          ...
          <xsl:for-each select="$available_items[generate-id() = generate-id(key('item-by-status-and-branch', concat(items:status, ' ', items:homebranch))[1])]">
            <xsl:value-of select="items:homebranch"/>
  			    <xsl:if test="items:itemcallnumber != '' and items:itemcallnumber"><xsl:text> </xsl:text>[<xsl:value-of select="items:itemcallnumber"/>]
  			    </xsl:if>
            <xsl:text> (</xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="count(key('item-by-status', 'reference'))>0">
        <span class="available">
          <b><xsl:text>Pour la consultation : </xsl:text></b>
