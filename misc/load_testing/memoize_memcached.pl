...
memoize_memcached('sysprefs', memcached => $memcached, expire_time => 600000); #cache for 10 minutes
my $query = new CGI;
...
sub sysprefs{
    my $dbh= DBI->connect("DBI:mysql:dbname=koha;host=vmsql;port=3306",
        "kohauser", "password");
    return $dbh->selectall_arrayref( "SELECT * from systempreferences LIMIT 0,10", {Slice =>{} });
}
