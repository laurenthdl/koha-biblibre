...
<xsl:stylesheet version="1.0" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="tag_4xx">
    <span class="results_summary">
    <xsl:if test="marc:datafield[@tag=461 or @tag=463]">
    <span class="label">Extrait de : </span> 
    <xsl:for-each select="marc:datafield[@tag=461 or @tag=463]">
	<xsl:if test="marc:subfield[@code='a']">
          <xsl:value-of select="marc:subfield[@code='a']"/>
	  <xsl:text> </xsl:text>
        </xsl:if>
	  ...
	  <a>
	  <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=ti:<xsl:value-of select="marc:subfield[@code='t']"/></xsl:attribute>
