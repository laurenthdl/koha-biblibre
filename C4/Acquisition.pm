...
BEGIN {
    ...
    @EXPORT = qw(
      ...
      &TransferOrder &GetCancelledOrders &GetOrderNumber &GetLateOrders
...
sub ModReceiveOrder {
    ...
    if ( $order->{quantity} > $quantrec ) {
        ...
        $order->{'orderstatus'} = 2;
        my ($basketno, $newordernumber) = NewOrder($order, $$order{parent_ordernumber});
