...
foreach my $rule (@$rules) {
    print "\nrule: itype " . $rule->{'itype'} . " / duration : " . $rule->{'duration'} . "\n";

    my $query = "select biblionumber, itemnumber from items where itype=? ";
    # $query .= "and itype=? " if ($ccode);
    $query .= "and TO_DAYS(NOW()) - TO_DAYS(dateaccessioned) >= ?"; 
    my $sth = $dbh->prepare($query);
    ($ccode) ? $sth->execute($rule->{'ccode'},  $conf->{'itype_new_value'}, $rule->{'duration'}) : $sth->execute($rule->{'itype'}, $rule->{'duration'});
