...
while ( my ($table, $field) = each %$level1_tables ) {
    ...
    push @trigger_str, "    CALL PROC_UPDATE_ID($table, NEW.borrowernumber, NEW.cardnumber);";
    ...
    push @trigger_str, "  FOR EACH ROW BEGIN";
    push @trigger_str, "    CALL PROC_DELETE_FROM($table, OLD.$field);";
    ...
    # Trigger update
    # update l'enregistrement avec le bon id
    push @trigger_str, "DROP TRIGGER IF EXISTS `TRG_BEF_UPD_$table` //";
    push @trigger_str, "CREATE TRIGGER `TRG_BEF_UPD_$table` BEFORE UPDATE ON `$table`";
    push @trigger_str, "  FOR EACH ROW BEGIN";
    push @trigger_str, "    DECLARE tmp_id INT(11);";
    push @trigger_str, "    CALL PROC_GET_NEW_ID($table, OLD.$field, tmp_id);";
...
while ( my ($table, $hash) = each %$level2_tables ) {
    ...
    push @trigger_str, "    DECLARE tmp_id INT(11);";
        while (my ($field, $ref) = each %$hash) {
            ...
            push @trigger_str, "    CALL PROC_GET_NEW_ID($table_referer, NEW.$field_referer, tmp_id);";
            push @trigger_str, "    SET NEW.$field = tmp_id;";
        }
    push @trigger_str, "  END;";
    push @trigger_str, "//";
...
#    # Trigger delete
#    # supprime l'enregistrement qui doit être supprimé et génère une erreur sur le delete courant
#    push @trigger_str, "DROP TRIGGER IF EXISTS `TRG_DEL_$table` //";
#    push @trigger_str, "CREATE TRIGGER `TRG_DEL_$table` BEFORE DELETE ON `$table`";
#    push @trigger_str, "  FOR EACH ROW BEGIN";
#        while (my ($field, $ref) = each %$hash) {
#            my ($table_referer, $field_referer) = split /\./, $ref;
#            push @trigger_str, "    CALL PROC_DELETE_FROM($table_referer, OLD.$field_referer);";
#            push @trigger_str, "    SELECT 'a' INTO @tmp;";      # Generate error
#            push @trigger_str, "    SELECT 'b', 'c' INTO @tmp;"; # mysql can't stop trigger normally
#        }
#    push @trigger_str, "  END;";
...
#
#    # Trigger update
#    # update l'enregistrement avec le bon id de la clé étrangère
#    push @trigger_str, "DROP TRIGGER IF EXISTS `TRG_UPD_$table` //";
#    push @trigger_str, "CREATE TRIGGER `TRG_UPD_$table` BEFORE UPDATE ON `$table`";
#    push @trigger_str, "  FOR EACH ROW BEGIN"
#        while (my ($field, $ref) = each %$hash) {
#            my ($table_referer, $field_referer) = split /\./, $ref;
#            push @trigger_str, "    DECLARE tmp_id INT(11);";
