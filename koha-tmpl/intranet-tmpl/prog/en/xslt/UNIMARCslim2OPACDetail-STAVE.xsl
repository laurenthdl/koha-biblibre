    ...
    <xsl:if test="marc:datafield[@tag=700] or marc:datafield[@tag=701] or marc:datafield[@tag=702] or marc:datafield[@tag=710] or marc:datafield[@tag=711] or marc:datafield[@tag=712]">
      ...
      <xsl:for-each select="marc:datafield[@tag=700]">
            <a>
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?idx=au&amp;q=<xsl:value-of select="marc:subfield[@code='a']"/><xsl:text>&amp;op=and&amp;idx=au&amp;q=</xsl:text><xsl:value-of select="marc:subfield[@code='b']"/></xsl:attribute>
                </xsl:otherwise>
              </xsl:choose>
      ...
      <xsl:for-each select="marc:datafield[@tag=702]">
          ...
          <xsl:choose>
            ...
            <xsl:otherwise><xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?idx=au&amp;q=<xsl:value-of select="marc:subfield[@code='a']"/><xsl:text>&amp;op=and&amp;idx=au&amp;q=</xsl:text><xsl:value-of select="marc:subfield[@code='b']"/></xsl:attribute></xsl:otherwise>
	  </xsl:choose>
...
<xsl:if test="marc:datafield[@tag=225]">
    <xsl:for-each select="marc:datafield[@tag=225]">
      <span class="results_summary">
          <xsl:text>(</xsl:text>
	 <a>
        <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=se:<xsl:value-of select="marc:subfield[@code='a']"/></xsl:attribute>
	<xsl:value-of select="marc:subfield[@code='a']"/>
	</a>
	<xsl:text>)</xsl:text>
	<xsl:if test="marc:subfield[@code='d']">
...
<xsl:if test="marc:datafield[@tag=205]">
  ...
  <span class="label">Genre : </span>
    ...
    <xsl:if test="marc:datafield[@tag=615]">
    <span class="results_summary">
      <xsl:for-each select="marc:datafield[@tag=615]">
        <a>
          <xsl:choose>
            <xsl:when test="marc:subfield[@code=9]">
              <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=an:<xsl:value-of select="marc:subfield[@code=9]"/></xsl:attribute>
